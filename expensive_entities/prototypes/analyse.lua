...
for _, rec in pairs(data.raw.recipe) do
    ...
    for _, result in pairs(res) do
        ...
        if e_e.find_prototype(result.name) then
    ...
    if e_e.affect_science then
        if rec.subgroup == "science-pack" or string.find(rec.name, "%-science%-pack")then
            e_e.include_recipe(rec.name)
            --log("Include recipe: "..rec.name)
            goto cont
        end
        ...
        if rec.subgroup == "science-pack" or string.find(rec.name, "%-science%-pack") then
            e_e.check_science_pack[#e_e.check_science_pack+1] = rec.name
        end
    end
